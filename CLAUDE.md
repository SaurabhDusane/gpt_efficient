...
- **Learned router:** `learned_router.py` (labelling + training, pure where possible) and `router.LearnedRouter`. Labels are pseudo-reference (cheaper tier judged against the top tier's answer). Train only on `evals/router_train.jsonl` — never on eval items (a test enforces disjointness). Models are JSON, never pickles; retraining changes the cache namespace via the model-file hash. `escalated` in the trace means low-confidence routing escalation.
...
- **Results analysis:** `analysis.py` holds all statistics and research-question tables (pure, tested); `notebooks/results.ipynb` and `gpte findings` only display/write them — never compute results in the notebook. Commit the notebook with outputs cleared (a test checks). Findings never auto-write interpretation, and anything derived from a fake run (`run.json` `fake: true`) must carry the ILLUSTRATIVE banner. Stats knobs (margin, confidence, reference order, pairs) live in `[analysis]`.
...
`id, ts, query_hash, cache_status, cache_sim, tier, provider, model, tokens_in, tokens_out, cost_usd, latency_ms, compressed, tokens_saved, escalated, response_len, embed_tokens, summary_tokens, route_confidence, error`. Keep this in sync with `SPEC.md` §3.6 — if you change one, change both.
